(function() {
    ...
    function delay(ms) {
        ...
        async function performClicks() {
            console.log("Starting the click sequence...");
            ...
            for (const city of villageArray) {
                console.log("city");
...
// Execute the click sequence immediately on the first run
    performClicks().then(() => {
        // Start the interval to perform clicks every minute (60000 milliseconds)
        intervalId = setInterval(performClicks, 60000); // Repeat every minute
